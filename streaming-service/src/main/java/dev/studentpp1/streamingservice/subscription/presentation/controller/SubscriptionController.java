...
public class SubscriptionController {
    ...
    public ResponseEntity<CheckoutResult> subscribe(
        ...
        CheckoutResult result = subscriptionCommandHandler.handle(new SubscribeCommand(request, currentUser.getId()));
    ...
    @GetMapping("/my")
    public ResponseEntity<PageResult<dev.studentpp1.streamingservice.subscription.presentation.dto.response.UserSubscriptionDto>> getMySubscriptions(
            @AuthenticationPrincipal AuthenticatedUser currentUser,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        return ResponseEntity.ok(subscriptionQueryHandler.handle(new GetMySubscriptionsQuery(currentUser.getId(), page, size)));
    }
