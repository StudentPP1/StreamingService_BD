...
- Для асинхронного сценарію
  ...
  Не блокує основний потік, нотифікація відправляється в окремому thread після обробки платежу.
...
- Для синхронного сценарію
  ...
  Основний потік блокується до завершення нотифікації

### Поведінка при збоях
- Асинхронно: збій у обробнику не ламає основну операцію.
- Синхронно: виняток із `notifyFailed(...)` йде вгору та спричиняє rollback транзакції.
  В нашому випадку помилка проігнорується через `try-catch`.

### Зв'язаність між компонентами
- Асинхронний варіант зменшує зв'язаність: модулі взаємодіють через подію, адаптер знає лише про подію та `eventBus`.
- Синхронний варіант підвищує зв'язаність: є прямий виклик контракту нотифікації, адаптер знає про конкретний метод `notifyFailed(...)` порта.
...
- Синхронний підхід простіший, але гірший із-за можливих помилок.
...
- Повторна обробка стримується на рівні payment-flow (перевірки статусів/стану платежу).
- Окремого dedup-механізму в notification-компоненті немає, тому при повторній доставці події можливе повторне надсилання повідомлення.